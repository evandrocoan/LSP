                ...
                for language in config.languages:
                    for scope in language.scopes:
                        if point is not None:
                            ...
                            if score > 0:
                                log(4, 'scope match score', scope, config.name, score)
                            if score > scope_score:
                                scope_score = score
                                scope_client_config = config
    log(4, 'chose ', scope_client_config.name if scope_client_config else None)
...
def apply_window_settings(client_config: 'ClientConfig', window: 'sublime.Window') -> 'ClientConfig':
    ...
    if client_config.name in window_config:
        ...
        log(2, 'window {} has override for {}'.format(window.id(), client_config.name), overrides)
